...
local Utility = {}
local Objects = {}
function Perc:DraggingEnabled(frame, parent)
        
    parent = parent or frame
    
    -- Made by Thrive, Wlourn
...
function Perc.CreateLib(PerName, themeList)
    ...
    local selectedTab 
    PerName = PerName or "Library"
    table.insert(Perc, PerName)
    for i,v in pairs(game.CoreGui:GetChildren()) do
        if v:IsA("ScreenGui") and v.Name == PerName then
            v:Destroy()
        end
    end
    ...
    Perc:DraggingEnabled(MainHeader, Main)
    ...
    function Perc:ChangeColor(prope,color)
